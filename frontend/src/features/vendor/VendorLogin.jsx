...
import { Mail, Lock, ArrowRight, ShieldCheck, ShoppingCart, Info, User, Phone, Building, MapPin } from 'lucide-react';
import useAuthStore from '../../store/useAuthStore';

const VendorLogin = () => {
  // Navigation & Auth Store Hooks
    ...
    if (isSignUp) {
      ...
      setTimeout(() => {
        ...
        const registeredVendor = {
          name: name.trim(),
          school: storeName.trim(),
          role: 'vendor',
          phone: phone.trim(),
          email: email.trim(),
          location: location.trim(),
          refId: `SEM-VEN-${Math.floor(100000 + Math.random() * 900000)}`
        };
        // Authenticate immediately
        login(registeredVendor, 'new-vendor-token');
      ...
      setTimeout(() => {
        setIsLoading(false);
        // Default login helper for quick evaluation if user leaves fields blank
        const authenticatedVendor = {
          name: 'Harsh',
          school: "Harsh's Hub",
          role: 'vendor',
          phone: '6268423925',
          email: email.trim(),
          refId: 'SEM-VEN-482019'
        };
        login(authenticatedVendor, 'mock-vendor-token');
        navigate('/vendor/dashboard');
      }, 1000);
    }
  };
