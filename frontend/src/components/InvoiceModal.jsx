...
import { X, Printer, Building2, User, FileText, AlertCircle } from 'lucide-react';
...
const InvoiceModal = ({ isOpen, onClose, invoiceData, loading, error }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200 print:bg-white print:p-0 print:block">
      {/* Print CSS Rules to isolate invoice receipt on window.print() */}
      <style>{`
        @media print {
          body * {
                    ...
                    {items.map((item, index) => {
                      ...
                      return (
                        <tr key={index} className="hover:bg-gray-50/50">
                          ...
                          <td className="py-3.5 px-4 text-center font-bold">{item.quantity || 1}</td>
                          <td className="py-3.5 px-4 text-right">{formatCurrency(item.pricePaise)}</td>
                          <td className="py-3.5 px-4 text-right text-gray-500">{formatCurrency(item.taxPaise || 0)}</td>
                          <td className="py-3.5 px-4 text-right font-bold text-gray-900">{formatCurrency(lineTotal)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
