...
import {
  ...
  approvePayoutRequest, rejectPayoutRequest,
                          ...
                          return (
                            <tr key={p.id} className="hover:bg-gray-50/50">
                              ...
                              <td className="px-5 py-4 text-gray-500 font-medium">
                                {p.bank?.bankName || '—'}
                                <span className="block text-[9px] text-gray-400">{p.bank?.ifsc || ''}</span>
                              </td>
                                ...
                                {isPending ? (
                                  <div className="flex items-center justify-end gap-2">
                                    <button onClick={() => handleApprove(p.id)} disabled={busy}
                                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase tracking-wider disabled:opacity-60">
                                      {busy ? <Loader2 size={12} className="animate-spin" /> : <CheckCircle2 size={12} />} Approve
                                    </button>
                                    <button onClick={() => handleReject(p.id)} disabled={busy}
                                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-100 text-[10px] font-black uppercase tracking-wider disabled:opacity-60">
                                      <XCircle size={12} /> Reject
                                    </button>
                                  </div>
                                ) : (
                                  <span className="text-[10px] text-gray-400 font-bold">{p.reference || '—'}</span>
                                )}
                              </td>
                            </tr>
                          );
